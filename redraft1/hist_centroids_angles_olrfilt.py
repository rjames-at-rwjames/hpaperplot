...
olrfilt=True
howolr='thlim' # type of OLR mean filter
...
for t in range(nthresh):
    ...
    for d in range(ndset):
        ...
        for mo in range(nmod):
            ...
            if olrfilt:
                if howolr=='thlim':
                    print 'Number before OLR filter'
                    print len(edts)
                    thtest=float(thisthresh)
                    pick3=np.where(olrmns < thtest)[0]
                    pick=pick3
                    edts=edts[pick]
                    cXs=cXs[pick]
                    cYs=cYs[pick]
